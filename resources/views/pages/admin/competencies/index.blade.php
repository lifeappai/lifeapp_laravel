    ...
    <div class="breadcrumb-header justify-content-between">
        <div>
            ...
            <nav aria-label="breadcrumb">
                <h4 class="content-title mb-2">{{ __('Competencies') }} ({{ $laCompetencies->total() }})</h4>
                <ol class="breadcrumb">
                ...
                <div class="card-body">
                            ...
                            <div class="d-flex">
                                ...
                                <div class="col-md-2">
                                    <a class="form-control btn btn-success"
                                        href="{{ route('admin.competencies.create') }}">Add Competency</a>
                                </div>
                            </div>
                        </form>
                    </div>
                    <div class="table-responsive">
                        <table id="example" class="table key-buttons text-md-nowrap">
                            ...
                            <?php $i = 1; ?>
                            <tbody>
                                @if (count($laCompetencies) > 0)
                                    @foreach ($laCompetencies as $laCompetency)
                                        <tr>
                                            <td>{{ $laCompetency->subject ? $laCompetency->subject->default_title : '' }}
                                            </td>
                                            <td>{{ $laCompetency->laLevel ? $laCompetency->laLevel->default_title : '' }}
                                            </td>
                                            <td>{{ $laCompetency->title }}</td>
                                            <td>{{ $laCompetency->document }}</td>
                                            <td>
                                                @if (App\Enums\StatusEnum::DEACTIVE == $laCompetency->status)
                                                    <button type="button"
                                                        class="btn btn-sm btn-danger waves-effect waves-light"
                                                        onclick="sweetAlertAjax('PATCH','{{ route('admin.competencies.status', $laCompetency->id) }}', 'Are You Sure To Publish')">Draft</button>
                                                @else
                                                    <button type="button"
                                                        class="btn btn-sm btn-success waves-effect waves-light"
                                                        onclick="sweetAlertAjax('PATCH','{{ route('admin.competencies.status', $laCompetency->id) }}', 'Are You Sure To Draft')">Publish</button>
                                                @endif
                                            </td>
                                            <td>
                                                <div class="d-flex">
                                                    <a href="{{ route('admin.competencies.edit', $laCompetency->id) }}"><button
                        ...
                        {{ $laCompetencies->appends(Request::all())->links() }}
                    </div>
                </div>
            </div>
        </div>
