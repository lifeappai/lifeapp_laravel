...
@section('content')
    <div class="row">
        <div class="col-lg-12 col-md-12">
            <div class="card">
                <div class="card-body">
                    <form method="POST" action="{{ route('admin.lession.plans.store') }}" id="competencyForm"
                            ...
                            <div class="col-md-4  mg-b-20">
                                ...
                                <select name="la_lession_plan_language_id" class="form-control" required>
                                    ...
                                    @foreach ($laLessionPlanLanguages as $key => $laLessionPlanLanguage)
                                        <option value="{{ $laLessionPlanLanguage->id }}">{{ $laLessionPlanLanguage->name }}
                                        </option>
                                    @endforeach
                                </select>
                            </div>
                            <div class="col-md-4  mg-b-20">
                                ...
                                <select name="type" class="form-control" required>
                                    ...
                                    @foreach (App\Enums\LessionPlanCategoryEnum::Category as $key => $category)
                                        <option value="{{ $category }}">{{ ucwords(str_replace('_', ' ', strtolower($key))) }}
                                        </option>
                                    @endforeach
                                </select>
                            </div>
