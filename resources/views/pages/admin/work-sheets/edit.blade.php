    ...
    <div class="breadcrumb-header justify-content-between">
        <div>
            ...
            <nav aria-label="breadcrumb">
                <h4 class="content-title mb-2">{{ __('Work Sheets') }}</h4>
                <ol class="breadcrumb">
...
@section('content')
    <div class="row">
        <div class="col-lg-12 col-md-12">
            <div class="card">
                <div class="card-body">
                    <form method="POST" action="{{ route('admin.work.sheets.update', $laWorkSheet->id) }}"
                        ...
                        <div class="row mb-3 mt-3">
                                ...
                                <select name="la_subject_id" class="form-control" required>
                                    ...
                                    @foreach ($subjects as $key => $subject)
                                        <option value="{{ $subject->id }}"
                                            @if ($laWorkSheet->la_subject_id == $subject->id) selected @endif>{{ $subject->default_title }}
                                        </option>
                                    @endforeach
                                </select>
                            </div>
                            <div class="col-md-6 mg-b-20">
                                ...
                                <select name="la_grade_id" class="form-control" required>
                                    ...
                                    @foreach ($laGrades as $key => $laGrade)
                                        <option value="{{ $laGrade->id }}" @if ($laWorkSheet->la_grade_id == $laGrade->id) selected @endif>{{ $laGrade->name }}</option>
                                    @endforeach
                                </select>
                            </div>
                            ...
                            <div class="col-md-2  mg-b-20">
                                @if (isset($laWorkSheet->media))
                                    <a target="_blank" href="{{ $imageBaseUrl . $laWorkSheet->media->path }}">
                                        View Document
                                    </a>
                                @endif
                            </div>
