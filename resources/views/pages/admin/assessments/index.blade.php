    ...
    <div class="breadcrumb-header justify-content-between">
        <div>
            ...
            <nav aria-label="breadcrumb">
                <h4 class="content-title mb-2">{{ __('Assessments') }} ({{ $laAssessments->total() }})</h4>
                <ol class="breadcrumb">
                ...
                <div class="card-body">
                            ...
                            <div class="d-flex">
                                ...
                                <div class="col-md-2">
                                    <a class="form-control btn btn-success"
                                        href="{{ route('admin.assessments.create') }}">Add Assessment</a>
                                </div>
                            </div>
                        </form>
                    </div>
                    <div class="table-responsive">
                        <table id="example" class="table key-buttons text-md-nowrap">
                            ...
                            <?php $i = 1; ?>
                            <tbody>
                                @if (count($laAssessments) > 0)
                                    @foreach ($laAssessments as $laAssessment)
                                        <tr>
                                            <td>{{ $laAssessment->subject ? $laAssessment->subject->default_title : '' }}
                                            </td>
                                            <td>{{ $laAssessment->laGrade ? $laAssessment->laGrade->name : '' }}
                                            </td>
                                            <td>{{ $laAssessment->title }}</td>
                                            <td>
                                                @if (isset($laAssessment->media))
                                                    <a target="_blank"
                                                        href="{{ $imageBaseUrl . $laAssessment->media->path }}">
                                                        View Document
                                                    </a>
                                                @endif
                                            </td>
                                            <td>
                                                @if (App\Enums\StatusEnum::DEACTIVE == $laAssessment->status)
                                                    <button type="button"
                                                        class="btn btn-sm btn-danger waves-effect waves-light"
                                                        onclick="sweetAlertAjax('PATCH','{{ route('admin.assessments.status', $laAssessment->id) }}', 'Are You Sure To Publish')">Draft</button>
                                                @else
                                                    <button type="button"
                                                        class="btn btn-sm btn-success waves-effect waves-light"
                                                        onclick="sweetAlertAjax('PATCH','{{ route('admin.assessments.status', $laAssessment->id) }}', 'Are You Sure To Draft')">Publish</button>
                                                @endif
                                            </td>
                                            <td>
                                                <div class="d-flex">
                                                    <a href="{{ route('admin.assessments.edit', $laAssessment->id) }}"><button
                        ...
                        {{ $laAssessments->appends(Request::all())->links() }}
                    </div>
                </div>
            </div>
        </div>
