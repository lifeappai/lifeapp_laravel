...
@section('content')
    <div class="row">
        <div class="col-lg-12 col-md-12">
            <div class="card">
                <div class="card-body">
                    <form method="POST" action="{{ route('admin.concept.cartoons.update', $laCartoonConcept->id) }}"
                        id="cartoonConceptForm" enctype="multipart/form-data">
                        @csrf
                        @method('put')
                        <div class="row mb-3 mt-3">
                                ...
                                <select name="la_subject_id" class="form-control" required>
                                    ...
                                    @foreach ($subjects as $key => $subject)
                                        <option value="{{ $subject->id }}"
                                            @if ($laCartoonConcept->la_subject_id == $subject->id) selected @endif>{{ $subject->default_title }}
                                        </option>
                                    @endforeach
                                </select>
                            </div>
                                ...
                                <select name="la_level_id" class="form-control" required>
                                    ...
                                    @foreach ($laLevels as $key => $laLevel)
                                        <option value="{{ $laLevel->id }}"
                                            @if ($laCartoonConcept->la_level_id == $laLevel->id) selected @endif>{{ $laLevel->default_title }}
                                        </option>
                                    @endforeach
                                </select>
                            </div>
                            ...
                            <div class="col-md-2  mg-b-20">
                                @if (isset($laCartoonConcept->media))
                                    <a target="_blank" href="{{ $imageBaseUrl . $laCartoonConcept->media->path }}">
                                        View Document
                                    </a>
                                @endif
                            </div>
