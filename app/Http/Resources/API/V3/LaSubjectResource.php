...
class LaSubjectResource extends JsonResource
{
    ...
    public function toArray($request)
    {
        ...
        if (Auth::user()) {
            $couponCodeUnlock = LaSubjectCouponCode::where('user_id', Auth::user()->id)
                ->where('la_subject_id', $this->id)
                ->first();
            if ($couponCodeUnlock) {
        ...
        return [
            ...
            'heading' => $this->heading['en'] ?? null,
            'meta_title' => $this->meta_title, // ✅ Added English meta title
            'image' => $image['en'] ?? null,
